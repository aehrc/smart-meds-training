...
async function injectClinicalData(patientId) {
  ...
  // MedicationRequest — Reaptan TPUU (safe)
  await post("MedicationRequest", {
    resourceType: "MedicationRequest",
    status: "active",
    intent: "order",
    medicationCodeableConcept: {
      coding: [{ system: "http://snomed.info/sct", code: "925679011000036105", display: "Reaptan 10 mg/10 mg (perindopril arginine/amlodipine) tablet" }],
...
/**
 * Ensure the given patient has AU clinical test data.
 * Called from the dashboard route after SMART auth completes.
 * This handles the case where the student selected Li Wang in the launcher
 * but the data was cleared, OR where the startup injection failed.
 */
async function ensurePatientData(patientId) {
  try {
    const medsResp = await fetch(
      `${FHIR_SERVER}/MedicationRequest?patient=${patientId}&status=active&_count=1`,
      { headers: HEADERS }
    ...
    if (hasMeds) {
      console.log(`[test-data] Patient/${patientId} already has medication data`);
      return;
    }

    console.log(`[test-data] Injecting clinical data for Patient/${patientId}...`);
    await injectClinicalData(patientId);
  } catch (err) {
    console.error("[test-data] Warning: could not ensure patient data:", err.message);
  }
}

module.exports = { ensureTestData, getTrainingPatientId, ensurePatientData };
